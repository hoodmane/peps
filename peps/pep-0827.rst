...
Discussions-To: https://discuss.python.org/t/pep-827-type-manipulation/106353
Status: Draft
...
types when possible** (example?).  This means inferring literal types for
arguments that **do not** appear in the bound, as well as
for arguments that **do** appear in the bound as read-only.

Each optional read-only item in the bound that does **not** have a
matching argument provided will
...
We introduce a ``Param`` type that contains all the information about a function param::
    ...
    class Param[N: str | None, T, Q: ParamQuals = typing.Never]: # Why Never default??
        pass
...
1. Explain here why name is needed for "a" positional only
2. Explain here why name is not needed for args & kwargs 
...
How different can equivalent types be? Because I've been pretty surprised in
typescript by things that are mutually assignable.
...
What does packed in a tuple mean? If I do GetArg[Callable[[int, str], int],
Callable, 0] I get tuple[int, str]? and to get str, I'd need to do GetArg[..., 1]?
...
* ``GetSpecialAttr[T, Attr: Literal[str]]``: Extracts the value
  ...
  ``GetSpecialAttr[Literal[1], "__name__"]`` should produce ``Literal["int"]``.
    ...
    Are any of these operators possible at runtime? For instance could I get
    `__name__` via actually T.__name__? Is that a bad idea? I guess for runtime
    type handling?
...
Is there an analogue of this in typescript?
...
Why not any way to make new *generic* types? I can't make SingleFieldDataClass[Literal["field_name"], FieldType]?
...
Is there an analogous thing in typescript?
...
Maybe this should be after we learn about the builtin operations??
